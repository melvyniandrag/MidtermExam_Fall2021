...
\iknow{cp -r}{Copy directory}
\iknow{mv}{moves a directory}
\iknow{rm}{remove directory
\iknow{rm -r {delete a directory}
\iknow{ls -a}{lists all files/hidden files}
\iknow{ls -al}{lists directories and hidden files}
\iknow{ls -l}{long listing format}
\iknow{ssh}{connection between a network}
\iknow{ssh-keygen}{reation of a public key}
\iknow{cd}{home directory}
\iknow{cd ../}{move one up in the directory}
\iknow{cd \}{navigate to a specific directory}
\iknow{echo}{displays output text }
\iknow{cat}{view directory data}
\iknow{adduser}{add a new user}
\iknow{deluser}{deletes a user}
\iknow{chmod}{change permissions}
\iknow{chown}{change ownership}
