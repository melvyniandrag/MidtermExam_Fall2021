...
\iknow{mv}{to rename a file}
\iknow{rm}{to remove a file}
\iknow{rm -r}{to remove a directory}
\iknow{ls -a}{to view all hidden files}
\iknow{ls -al}{to list all the files including hidden}
\iknow{ls -l}{to list files}
\iknow{ssh}{secure shell used to connect to a remote server}
\iknow{ssh-keygen}{to create a ssh key}
\iknow{cd}{to change directories}
\iknow{cd ../}{to go to the last directory you were in}
\iknow{cd \textasciitilde}{to change to textasciitilde folder}
\iknow{echo}{to display a message}
\iknow{cat}{to view typically whats inside of a file}
\iknow{adduser}{to add a user}
\iknow{deluser}{to delete a user}
\iknow{chmod}{changes permissions on a file}
