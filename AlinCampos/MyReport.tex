...
\iknow{cp -r}{copies a directory}
\iknow{mv}{renames a file or directory}
\iknow{rm}{deletes a file}
\iknow{rm -r}{deletes file or directory}
\iknow{ls -a}{shows all files even hidden files in directory}
\iknow{ls -al}{shows permissions, owner, groups, size, time, name, and hidden files}
\iknow{ls -l}{shows permissions, owner, groups, size, time, name}
\iknow{ssh}{connect to remote linux machine}
\iknow{ssh-keygen}{generates a key}
\iknow{cd}{changes current directory}
\iknow{cd ../}{goes up one directory}
\iknow{cd \textasciitilde}{goes directly to home directory}
\iknow{echo}{repeats what is written}
\iknow{cat}{shows contents of a file}
\iknow{adduser}{adds a user to linux}
\iknow{deluser}{deletes a user}
\iknow{chmod}{changes access permissions}
\iknow{chown}{changes file owners and groups}
