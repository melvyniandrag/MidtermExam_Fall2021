...
\iknow{cp -r}{copying directories including everything inside of them}
\iknow{mv}{renames or moves files/directories}
\iknow{rm}{used for removing files}
\iknow{rm -r}{used for removing directories and everything inside of them}
\iknow{ls -a}{lists all files including hidden files}
\iknow{ls -al}{lists all file permissions and other metadata including hidden files}
\iknow{ls -l}{lists all file permissions and other metadata excluding hidden files}
\iknow{ssh}{allows user to connect to a remote server}
\iknow{ssh-keygen}{creates an ssh key-gen pair}
\iknow{cd}{changes directory}
\iknow{cd ../}{goes back to the parent directory}
\iknow{cd \textasciitilde}{goes back to the home directory}
\iknow{echo}{outputs variable or expression}
\iknow{cat}{used to view, create, and concatenate files}
\iknow{adduser}{adds a user to the system and creates a home directory for that user}
\iknow{deluser}{deletes a user from the system and deletes associated files}  
\iknow{chmod}{changes file/directory permissions}
\iknow{chown}{changes ownership of files/directories}
