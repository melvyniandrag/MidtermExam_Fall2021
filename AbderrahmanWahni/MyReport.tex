...
\iknow{mv}{rename a file or directory}
\iknow{rm}{to delete a file}
\iknow{rm -r}{to delete a directory or file}
\iknow{ls -a}{to show the hidden files}
\iknow{ls -al}{to list the files and directories with detailed information like the permissions, size, owner, etc.}
\iknow{ls -l}{shows file or directory, size, modified date and time, file or folder name and owner of the file, and its permission.}
\iknow{ssh}{to Remotely log in to another Linux machine}
\iknow{ssh-keygen}{to generate a key}
\iknow{cd}{to change directory}
\iknow{cd ../}{to move to the next directory}
\iknow{cd \textasciitilde}{to change to /textasciitilde directory}
\iknow{echo}{to display a line}
\iknow{cat}{Display file’s contents to the standard output device}
\iknow{adduser}{to add new user}
\iknow{deluser}{to delete a user}
\iknow{chmod}{to change files permissions}
\iknow{chown}{to change file ownership}
