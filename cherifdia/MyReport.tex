...
\title{how to get A in thisn class}
...
\iknow{cp}{copy}
\iknow{cp -r}{copy multiple}
\iknow{mv}{rename}
\iknow{rm}{delete}
\iknow{rm -r}{delete multiple}
\iknow{ls -a}{list hidden file/dir}
\iknow{ls -al}{list hidden file and decription}
\iknow{ls -l}{list hidden description}
\iknow{ssh}{secure shell}
\iknow{ssh-keygen}{gerate keygen}
\iknow{cd}{change dir}
\iknow{cd ../}{back one dir}
\iknow{cd \textasciitilde}{to do}
\iknow{echo}{create file and modifier}
\iknow{cat}{display content}
\iknow{adduser}{add account user}
\iknow{deluser}{delete account}
\iknow{chmod}{change mode rwx}
\iknow{chown}{change pwner}
